...
Based on the matrix it appears that the percent of white people has a larger positive impact on turnout and the percent of unemployed people has a higher negative impact on turnout. The variables that seem to be postively correlated are the percent of white people and income. it appears that some categories that might not have an impact are 
...
The min and max tell us that there is not much variation between the data which means that it is more likely to be accurate compared to the other data. The coefficients show that there is a higher change of income, median age and percentage of white people having a positive impact on the turnout. The coefficients also show that there is a negative impact on turnout based on population, percent of bachelors and percent unemployed. The p-value is also high as well as the adjusted R which suggests that there is statistical significance with the data. 
...
None of my models were more successful than the original model we used with everything. The model that was most successful was model2 where I used median age, population, income and the pct of white people. This model had a R squared value of 0.7997. The closest after this was model4 with a R squared of 0.74. The worst model by far was model3 with a R squared value of 0.53. This model had only the percent unemployed, income and pct of bachelors higher which tells me these categories have less of an impact. 

```{r}
# Model 2
model2 <- lm(turnout ~ median_age + population + income + pct_white, data = md_turnout)
summary(model2)

# Model 3
model3 <- lm(turnout ~ pct_unemployed + income +pct_bachelors_higher, data = md_turnout)
summary(model3)

# Model 4
model4 <- lm(turnout ~ population + pct_white + income, data = md_turnout)
summary(model4)
...
I could use this data for an election story to see how much turnout there will be in certain locations based on the demographics and people that live in these areas. This would help with gathering a consensus and figuring out how many people from each area/county will be voting. Once you know roughly how many people are expected to vote based off of this data you could start gathering polls to see how people are responding and compare the results to the entire sample size you estimate from the model. This would give you a rough idea of how votes could be cast and show you what locations will be the most influential in the election race. 
